# Code adapted from stable-baselines
...
def squashed_gaussian_likelihood(input_, mu_, log_std):
    return gaussian_likelihood(input_, mu_, log_std) - tf.reduce_sum(tf.log(1 - input_ ** 2 + EPS), axis=1)
...
class FeedForwardPolicy(SACPolicy):
    ...
    def kl_with_replay(self, replay_mu, replay_std):
        return tf.reduce_sum(tf.math.log(replay_std) - tf.math.log(self.std) + (tf.square(self.std) + tf.square(self.act_mu - replay_mu)) / (2.0 * tf.square(replay_std)) - 0.5, axis=-1)

    def kl_with_replay_reverse(self, replay_mu, replay_std):
        return tf.reduce_sum(tf.math.log(self.std) - tf.math.log(replay_std) + (tf.square(replay_std) + tf.square(replay_mu - self.act_mu)) / (2.0 * tf.square(self.std)) - 0.5, axis=-1)
    ...
    def step_with_probs(self, obs, state=None, mask=None, deterministic=False):
        if deterministic:
            return self.sess.run(self.deterministic_policy, {self.obs_ph: obs}), None, None
        return self.sess.run([self.policy, self.logp_pi, self.act_mu, self.std], {self.obs_ph: obs})

    def action_likelihood(self, actions):
        return gaussian_likelihood(actions, self.act_mu, tf.math.log(self.std)) - tf.reduce_sum(tf.log(1 - actions ** 2 + EPS), axis=1)

    def get_irm_logp_pi(self, dummy):
        new_logp_pi = gaussian_likelihood(dummy * self.unsquashed_policy, self.act_mu, tf.log(self.std))
        _, _, new_logp_pi = apply_squashing_func(self.act_mu, dummy * self.unsquashed_policy, new_logp_pi)
        return new_logp_pi
